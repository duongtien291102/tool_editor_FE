import { ListPlus, Loader2, Plus, Save, Send, Sparkles, Trash2 } from 'lucide-react';
...
function loadDraft(projectId: string, projectName: string): ManualScriptDraft {
  try {
...
export function ManualScriptWorkspace({
  ...
  const [draft, setDraft] = useState(() => loadDraft(projectId, projectName));
  ...
  const markGenerationStarted = useProductionFlowStore(
  ...
  useEffect(() => {
    ...
    const localDraft = loadDraft(projectId, projectName);
    setDraft(localDraft);
    void responseData(
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      localStorage.setItem(draftKey(projectId), JSON.stringify(next));
      void responseData(
          ...
          <div className="space-y-1">
            {draft.scenes.map((scene, index) => (
              <a
                key={scene.id}
                href={`#scene-${scene.id}`}
                className="block rounded-md border border-transparent px-3 py-2 hover:border-white/8 hover:bg-white/[0.03]"
              >
