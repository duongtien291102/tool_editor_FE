...
export const StatusBar: React.FC = () => {
  ...
  const getStatusText = () => {
    switch (status) {
      case SaveStatus.Saving: return t('status.saving');
      case SaveStatus.Saved: return t('status.saved');
      case SaveStatus.Dirty: return t('status.dirty');
      case SaveStatus.Error: return t('status.error');
      default: return '';
    }
  };

  return (
    <div className="h-8 border-t border-border bg-card flex items-center justify-between px-4 text-xs text-muted-foreground shrink-0 select-none">
